...
export default function Dashboard() {
  ...
  // 3. Ajoute ces deux fonctions pour changer de page
  const nextSlide = () => {
    // On avance de 2 en 2, mais on s'arrête avant la fin
    if (currentIndex < robots.length - 2) {
      setCurrentIndex(currentIndex + 2);
    }
  };

  const prevSlide = () => {
    // On recule de 2 en 2, sans descendre en dessous de 0
    if (currentIndex > 0) {
      setCurrentIndex(currentIndex - 2);
    }
  };
  ...
  return (
    <DashboardLayout>
                  ...
                  <div className="robot-info-line">
                    ...
                    <div className="progress-container">
                      <div className="progress-bar">
                        <div className="progress-fill" style={{ width: `${robot.battery}%` }}></div>
                      </div>
                      <span className="value">{robot.battery}%</span>
                    </div>
                  </div>

                  <div className="robot-info-line">
                    <span className="label temp-label">Température</span>
                    <span className="value">{robot.temp}°C</span>
                  </div>

                  <div className="robot-info-line">
                    <span className="label mode-label">Mode</span>
                    <span className="value">{robot.mode}</span>
                  </div>
      ...
      {/* Système */}
      <Card title="État Simulation & Système" span={1}>
